...
const azureDevopsAPI_1 = require("./helpers/azureDevopsAPI");
//@ts-ignore
...
const argv = yargs(process.argv.slice(2))
    ...
    updateNpmrcWithNewPat(argv.npmrc);
})
    .parseAsync();
    ...
    files.map((file) => {
        ...
        if (fs.existsSync(`${NPMRC_STORE}/${file}`)) {
            ...
            if (authIdIsThere) {
                ...
                (0, azureDevopsAPI_1.revokePat)(azProject, authId);
                npmrc = npmrc.replace(`#script#${authId}#script#`, '');
            }
            // create new PAT
            const patToken = (0, azureDevopsAPI_1.createPat)(azProject, `${file}_feed`);
